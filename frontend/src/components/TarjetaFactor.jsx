    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden group relative">
      <div className="p-3 sm:p-4 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mb-2.5 pr-6">
          <span className="text-[10px] text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100 uppercase tracking-wide truncate">
            {factor.categoria}
          </span>
          <span className="text-[10px] text-gray-400 font-semibold bg-gray-50 px-2 py-0.5 rounded-full border border-gray-100 uppercase tracking-wide truncate">
            {factor.region}
          </span>
        </div>

        {/* Title / Subtype */}
        <h3 className="font-bold text-gray-900 text-sm sm:text-base leading-tight mb-2.5 truncate" title={factor.subtipo}>
          {factor.subtipo}
        </h3>
        ...
        <div className="bg-gradient-to-r from-emerald-50/50 to-teal-50/20 border border-emerald-100/50 rounded-lg p-2.5 mb-3">
          ...
          <div className="text-xs sm:text-sm font-bold text-emerald-950 truncate" title={`${factor.valor} kgCO2e / ${factor.unidad}`}>
            {factor.valor} <span className="text-[10px] sm:text-xs text-emerald-700 font-normal">kgCO2e / {factor.unidad}</span>
          </div>
        </div>

        {/* Metadata Details */}
        <div className="space-y-1.5 text-[11px] sm:text-xs text-gray-500 pt-2.5 border-t border-gray-100">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="shrink-0 text-gray-400" aria-hidden="true">📚</span>
            <span className="truncate" title={factor.fuente}>Fuente: {factor.fuente}</span>
          </div>
          <div className="flex items-center justify-between gap-2 text-[10px] text-gray-400 pt-0.5">
